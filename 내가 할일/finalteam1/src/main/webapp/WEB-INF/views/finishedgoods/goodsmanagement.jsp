...
window.addEventListener("resize", function() {
    ...
    if(document.getElementsByClassName('containerdiv')[1].getBoundingClientRect().width<463){
        document.getElementsByClassName('goodstitle')[1].style.display="none";
    } else{
        document.getElementsByClassName('goodstitle')[1].style.display="flex";
    }
});
...
goodsInsert.addEventListener('click', function(){
    ...
    goodsStatusChange(name, price, 1);
...
var goodsService = (function() {
    ...
    function update(goodsplus,callback,error){
        $.ajax({
            type:'put',
            url:'/goodsrest/name/'+goodsplus.name+'/price/'+goodsplus.price+'/status/'+goodsplus.status,
    ...
    function goodsRemove(gnum,callback,error){
        $.ajax({
            type:'delete',
            url:'/goodsrest/gnum/'+gnum,
...
function listDelete(gnum){
    var con=confirm("삭제하시겠습니까?");
    if(con===true){
        goodsService.goodsRemove(gnum, function(){
            alert("삭제했습니다.");
            showGoodsList(1);
        }, function(err){
            alert("error....");
...
function goodsStatusChange(name, price, status){
    console.log('상태',name, price, status)
    goodsService.update(
        {
            name:name,
            price:price,
